...
from src.api.incidents.enums import IncidentStatus
from src.api.database import Database
from pymongo import ReturnDocument

from src.api.incidents.model import IncidentAnomaly, Incident, IncidentCounterModel
...
class IncidentsDao:
    ...
    def __init__(self, tenant_id: str, tenant_namespace: str):
        ...
        self.collection = self.db.get_collection("incidents")
        self.counter_collection = self.db.get_collection("incident_counter")
    ...
    def update_incident(self, incident_id: str, status: int, owner_id: str,
                        stopped_at: Optional[datetime] = None,
                        anomalous_kpis: Optional[list[IncidentAnomaly]] = None) -> Optional[Incident]:
        ...
        try:
            ...
            updated_incident = self.collection.find_one_and_update(
        ...
        try:
            ...
            update_query = {
                "$set": {
                    ...
                    "anomalies.$.values": kpi_values,
                    "anomalies.$.latestTrend": latest_trend,
                    "anomalies.$.forecast": forecast,
                    "anomalies.$.overallTrend": overall_trend,
                    "updatedAt": datetime.now(UTC)
                }
            }
    ...
    def get_ongoing_or_in_observation_incident(self, asset_code: str) -> Optional[Incident]:
        """
        ...
        try:
            ...
            three_hours_ago = datetime.now(timezone.utc) - timedelta(hours=self.previous_incident_time_delta_in_hours)
            current_time = datetime.now(timezone.utc)

            existing_incident = self.collection.find_one({
                "assetCode": asset_code,
                "$or": [
                    {
                        "stoppedAt": {
                            "$gte": three_hours_ago,  # Greater than or equal to 3 hours ago
                            "$lte": current_time  # Less than or equal to now
                        }
                    },
                    {"stoppedAt": None}
                ],
                "status": {"$in": [IncidentStatus.ONGOING.value, IncidentStatus.IN_OBSERVATION.value]}
            }, sort=[("updatedAt", -1)])
    ...
    def get_next_incident_id(self, owner_id: str, asset_code: str) -> int:
        ...
        update = {"$inc": {"incident": 1}, "$setOnInsert": {"owner_id": owner_id, "asset_code": asset_code}}
