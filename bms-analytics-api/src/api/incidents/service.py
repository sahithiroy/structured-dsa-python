...
from src.api.incidents.alerts_service import AlertsService
from src.api.incidents.dao import IncidentsDao
from src.api.incidents.enums import BatteryKPI, IncidentStatus, ForecastSteps, BatteryNames, AlertCategory
from src.api.incidents.model import KPIPayload, Incident, IncidentAnomaly
from src.api.data.dao import IncidentData
from src.api.data.service import TimeseriesAnalyticsService


class IncidentsService:

    def __init__(self, tenant_id: str, tenant_namespace: str, siteCode: str, assetCode: str):
        """
        ...
        self.timeseries_analytics_service = TimeseriesAnalyticsService()

    def process_event(self, event: KPIPayload, owner_id: str) -> bool:
        """
    ...
    Args:
        event (KPIPayload): An object containing the details of the current and previous state
    ...
    """
        ...
        if event.prev.e_state == 'N' and event.now.e_state == 'E':
            logging.info(f"[Tenant-{owner_id}][{event.asset_code}][{event.battery_unit_id}] New error started for asset '{event.asset_code}'")
            try:
                # Handle the transition from normal state ('N') to error state ('E').
                self.state_transition_normal_to_error(payload_event=event, owner_id=owner_id)
                return True
        ...
        # Check if the event indicates an ongoing error (state remains 'Error').
            ...
            logging.info(f"[Tenant-{owner_id}][{event.asset_code}][{event.battery_unit_id}] Ongoing error continues for asset '{event.asset_code}'")
            try:
                # Handle the transition when the state remains in error ('E').
                self.state_transition_error_to_error(payload_event=event, owner_id=owner_id)
                # Get the anomalous KPIs to mention in the message
                return True
            except Exception as e:
                logging.info(
        ...
        # Check if the event indicates an error stopping (state changes from 'Error' to 'Normal').
            ...
            logging.info(f"[Tenant-{owner_id}][{event.asset_code}][{event.battery_unit_id}] An ongoing error stopped for asset '{event.asset_code}'")
            # Handle the transition from error state ('E') to normal state ('N').
            try:
                self.state_transition_error_to_normal(payload_event=event, owner_id=owner_id)
                return True
            except Exception as e:
                logging.info(
    ...
    def state_transition_normal_to_error(self, payload_event: KPIPayload, owner_id: str) -> Optional[Incident]:
        """
        ...
        existing_incident: Optional[Incident] = self.dao.get_ongoing_or_in_observation_incident(
        ...
        if existing_incident:
            ...
            incident_type = "ongoing" if existing_incident.status == 0 else "in-observation"

            # Extract the KPIs currently associated with the incident for the relevant battery and bank unit.
            current_battery_existing_anomalous_kpi_names: list[str] = list(map(
                lambda anomaly: anomaly.kpi,
                filter(
                    lambda anomaly: (
                                            anomaly.batteryUnitId == payload_event.battery_unit_id or anomaly.batteryUnitId == BatteryNames.BATTERIES.value) and
                                    anomaly.bankUnitId == payload_event.bank_unit_id,
            ...
            for kpi_name in current_battery_existing_anomalous_kpi_names:
                ...
                kpi_anomaly_objects: list[IncidentAnomaly] = list(filter(
                    lambda a: a.kpi == kpi_name and (
                            a.batteryUnitId == payload_event.battery_unit_id
                            or a.batteryUnitId == BatteryNames.BATTERIES.value),
                    existing_incident.anomalies
                ))
                kpi_anomaly_object: Optional[IncidentAnomaly] = kpi_anomaly_objects[0] if len(
                    kpi_anomaly_objects) > 0 else None
                if kpi_anomaly_object:
                    ...
                    if (
                            kpi_anomaly_object.kpi == BatteryKPI.CURRENT.value and payload_event.now.ci_as == kpi_anomaly_object.severity
                            and kpi_anomaly_object.batteryUnitId == BatteryNames.BATTERIES.value):
                        logging.info(
                        ...
                        continue
                    # Estimate the data trends (latest and overall) and forecast
                    # kpi_metrics, data_trends_and_forecast = self.get_battery_metrics_and_trends_with_forecast(
                    #     kpi_name, existing_incident.createdAt, datetime.now(), kpi_anomaly_object.bankUnitId,
                    #     kpi_anomaly_object.batteryUnitId
                    # )
                    # logging.info(
                    #     f"[Tenant-{owner_id}][{payload_event.asset_code}][{payload_event.battery_unit_id}] Fetched data trends and forecasts for the KPI: {kpi_name}"
                    # )
                    # Update the incident with anomaly details
                    self.dao.update_incident_anomaly(
                        incident_id=existing_incident.incidentId,  # Incident ID of the existing anomaly.
                        battery_unit_id=self.get_battery_name_by_kpi(kpi_name, payload_event),
                        # Get the battery unit ID.
                        kpi=kpi_name,  # KPI name.
                        severity=self.get_severity(kpi_name, payload_event),  # Determine the new severity.
                        owner_id=owner_id,  # Owner ID responsible for this anomaly.
                        kpi_values=[],  # kpi_metrics,  # Update with the latest anomaly values.
                        latest_trend="",  # data_trends_and_forecast.latest_trend,  # Update latest trend.
                        forecast=[],  # data_trends_and_forecast.forecast,  # Update forecasted values.
                        overall_trend=""  # data_trends_and_forecast.overall_trend  # Update overall trend.
                    )
                    logging.info(
                        f"[Tenant-{owner_id}][{payload_event.asset_code}][{payload_event.battery_unit_id}] Successfully updated the data trends and forecast of existing severities of existing incident : {existing_incident.incidentId}"
                    )

            # Identify new KPIs that were not previously part of the incident.
            to_add_anomalous_kpi_names = list(filter(
            ...
            for kpi_name in to_add_anomalous_kpi_names:
                # anomaly_values, data_trends_and_forecast = self.get_battery_metrics_and_trends_with_forecast(
                #     kpi_name, existing_incident.createdAt, datetime.now(), payload_event.bank_unit_id,
                #     payload_event.battery_unit_id
                # )

                # Create an IncidentAnomaly object for each new anomalous KPI.
                to_add_anomalous_objects.append(IncidentAnomaly(
                    kpi=kpi_name,
            ...
            return self.dao.update_incident(
                ...
                owner_id=owner_id,
                anomalous_kpis=to_add_anomalous_objects if to_add_anomalous_objects else None
            )

        # If no existing incident is found, create a new incident.
        else:
            logging.info(f"[Tenant-{owner_id}] Creating a new incident for asset '{payload_event.asset_code}'")
            ...
            next_incident_id = self.dao.get_next_incident_id(owner_id, payload_event.asset_code)
            ...
            new_incident = Incident(
                ...
                incidentId=f"INC-{payload_event.asset_code.upper()}-{next_incident_id}",  # Generate unique incident ID
    ...
    def state_transition_error_to_error(self, payload_event: KPIPayload, owner_id: str) -> Optional[Incident]:
        """
        ...
        severity_changed_kpi_names: list[str] = self.create_alerts_for_severity_changed_anomalous_kpis(
            owner_id=owner_id,
            event=payload_event
        )
        logging.info(
        ...
        if len(severity_changed_kpi_names) > 0:
            ...
            if existing_incident:
                ...
                current_battery_existing_anomalous_kpi_names: list[str] = list(
                    map(lambda x: x.kpi, filter(
                        lambda y: (
                                          y.batteryUnitId == payload_event.battery_unit_id or y.batteryUnitId == BatteryNames.BATTERIES.value) and
                                  y.bankUnitId == payload_event.bank_unit_id,
                ...
                for kpi_name in current_battery_existing_anomalous_kpi_names:
                    ...
                    if kpi_anomaly_object:
                        ...
                        # )
                        # logging.info(
                        #     f"[Tenant-{owner_id}][{payload_event.asset_code}][{payload_event.battery_unit_id}] Fetched data trends and forecasts for the KPI: {kpi_name} : {severity_changed_kpi_names}"
                        # )
                        # Update the incident with anomaly details
                        self.dao.update_incident_anomaly(
                            incident_id=existing_incident.incidentId,  # Incident ID of the existing anomaly.
                ...
                to_add_anomalous_objects = []
                for kpi_name in to_add_anomalous_kpi_names:
                    # For each anomaly => calculate the trends
                    # anomaly_values, data_trends_and_forecast = self.get_battery_metrics_and_trends_with_forecast(
                    #     kpi_name, existing_incident.createdAt, datetime.now(), payload_event.bank_unit_id,
                    #     payload_event.battery_unit_id
                    # )
            ...
            #
                ...
                new_anomalous_kpis = list(map(lambda x: IncidentAnomaly(
                    kpi=x,
                    severity=self.get_severity(x, payload_event),
                    batteryUnitId=self.get_battery_name_by_kpi(x, payload_event),
                    bankUnitId=payload_event.bank_unit_id,
                ), now_anomalous_kpi_names))

                # Generate a unique incident ID.
                next_incident_seq = self.dao.get_next_incident_id(owner_id, payload_event.asset_code)
                incident_id = f"INC-{payload_event.asset_code.upper()}-{next_incident_seq}"
                ...
                logging.info(
                    f"[Tenant-{owner_id}][{payload_event.asset_code}][{payload_event.battery_unit_id}] Successfully created a new incident with id - {incident_id}"
                )
                return result
        #
        ...
        #
            ...
            #
                ...
                new_incident = Incident(
                    ...
                    assetCode=payload_event.asset_code,
                    anomalies=new_anomalous_kpis,
                    incidentId=f"INC-{payload_event.asset_code.upper()}-{next_incident_id}",
                    ownerId=owner_id,
                    createdAt=datetime.now(UTC),
                    updatedAt=datetime.now(UTC),
                    stoppedAt=None,
                ...
                return self.dao.add_new_incident(incident=new_incident,
                                                 owner_id=owner_id)

    def state_transition_error_to_normal(self, payload_event: KPIPayload, owner_id: str) -> Optional[Incident]:
        """
        ...
        if existing_incident:
            return self.dao.update_incident(
                ...
                stopped_at=datetime.now(UTC),  # Record the current timestamp as the stop time for the incident.
                owner_id=owner_id
            )
    ...
    def create_alerts_for_anomalous_kpis(self, owner_id: str, event: KPIPayload, create_alert: bool = True) -> list[
        str]:
        """
        ...
        alerts_service = AlertsService(asset_code=event.asset_code, tenant_id=owner_id, site_code=event.site_code)
        ...
        if (event.now.ci_as or 0) > 0:
            ...
            existing_Incident_anomaly = existing_incident.anomalies if existing_incident else []

            # Extract bankUnitId and kpi values from existing anomalies
            bank_unit_ids = [anomaly.bankUnitId for anomaly in existing_Incident_anomaly]
            ...
            # Get the existing anomalies from the incident.
            existing_incident_anomalies = existing_incident.anomalies if existing_incident else []
            ...
            for anomaly in existing_incident_anomalies:
                if anomaly.kpi == BatteryKPI.CURRENT.value:  # Check if the anomaly is for 'Current'.
            ...
            if create_alert and (
                    (event.bank_unit_id not in bank_unit_ids and BatteryKPI.CURRENT.value not in kpi_values) or (
                    current_anomaly_severity != event.now.ci_as)):
                alerts_service.create_alert(
                    tenant_id=owner_id,
    ...
    def create_alerts_for_severity_changed_anomalous_kpis(self, owner_id: str, event: KPIPayload,
                                                          create_alert: bool = True) -> list[
        str]:
        """
        ...
        if (event.now.ci_as or 0) != (event.prev.ci_as or 0):
            ...
            current_anomaly_severity = None
            for anomaly in existing_incident_anomalies:
                if anomaly.kpi == BatteryKPI.CURRENT.value and anomaly.batteryUnitId == BatteryNames.BATTERIES.value:  # Check if the anomaly is for 'Current'.
                    current_anomaly_severity = anomaly.severity

            # If the severity of 'Charging Current' has changed and is greater than 0, create an alert.
            if (event.now.ci_as or 0) > 0 and create_alert and current_anomaly_severity != event.now.ci_as:
    ...
    def get_battery_metrics_and_trends_with_forecast(
            self, kpi_name: str, start_time: datetime, end_time, bank_id, battery_id
    ) -> Tuple[List[float], DataTrendsAndForecast]:
        """
        Retrieves anomaly-related values for a given KPI and incident.

        Args:
            kpi_name (str): The name of the KPI to fetch data for.
            incident_id (str): The ID of the incident.
            start_time (datetime): The start time of the anomaly.

        Returns:
            Tuple[List[float], DataTrendsAndForecast]:
            A tuple containing battery metric values and incident analysis results.
        """

        # Fetch battery metric values based on anomaly start time and KPI
        overall_battery_metrics: List[float] = self.incident_data.fetch_battery_metrics_for_given_time_range(
            start_time,
            end_time,
            kpi_name,
            bank_id,
        ...
        latest_battery_metrics: List[float] = self.incident_data.fetch_latest_battery_metrics_after_given_time(
            start_time,
            kpi_name
        )

        # Process incidents and forecast trends
        analysis_results: DataTrendsAndForecast = self.timeseries_analytics_service.process_timeseries_data(
            overall_battery_metrics, latest_battery_metrics, ForecastSteps.THIRTY.value
        )

        # Return both values and the incident results
        return overall_battery_metrics, analysis_results
    ...
    def get_severity(self, kpi_name: str, payload_event: KPIPayload) -> Optional[int]:
        ...
        if kpi_name == BatteryKPI.VOLTAGE.value:  # Voltage anomaly
            ...
            if payload_event.now.di_as > 0:  # Discharging current anomaly (overwrites ci_as if both exist)
    ...
    def get_battery_name_by_kpi(self, kpi_name: str, payload_event: KPIPayload) -> Optional[str]:
        """
        ...
        return BatteryNames.BATTERIES.value if kpi_name == BatteryKPI.CURRENT.value and payload_event.now.ci_as > 0 else payload_event.battery_unit_id
