...
class KPIDataPoint(BaseModel):
    ...
    v_as: Optional[int] = (
        None  # Voltage anomaly status, if any (1 for anomalous, 0 for normal).
    )
    ...
    e_state: Optional[str] = (
        None  # Error state indicator (e.g., 'E' for error, 'N' for normal).
    )
...
class IncidentAnomaly(BaseModel):
    ...
    values: Optional[list[float]] = (
        None  # The values of the above KPI during the incident duration.
    )
    latestTrend: Optional[str] = None  # Latest behavioral trend observed for the KPI.
    forecast: Optional[list[float]] = (
        None  # Forecasted behavioral trend estimated for the KPI.
    )
...
class Incident(BaseModel):
    ...
    status: (
        int  # Status of the incident: 0 → ongoing, 1 → under-observation, 2 → closed.
    )
    anomalies: Optional[list[IncidentAnomaly]] = (
        None  # List of anomalous battery + KPI information for this incident.
    )
