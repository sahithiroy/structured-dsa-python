...
class AlertsService:
    ...
    def create_alert(
        ...
        if kpi == BatteryKPI.VOLTAGE.value or kpi == BatteryKPI.STATE_OF_CHARGE.value:
            payload = {
                "kpi": kpi,
                "data": value,
                "severity": severity,
        ...
        logging.info(
            f"[Tenant-{tenant_id}] Sending alert creation request to {self.url} : {payload}"
        )
        # Make the POST request to create the alert
        try:
            headers = {
                "Content-Type": "application/json"
            }  # Define the headers (specifying JSON content type)
            ...
            if response.status_code == 200:  # If the response status is 200 (success)
                logging.info(
                    f"[Tenant-{tenant_id}] Alert created successfully : {response.json()}"
                )  # Log success
        ...
        ) as e:  # Catch any exceptions that occur during the request
            logging.error(
                f"[Tenant-{tenant_id}] An error occurred while creating the alert: {e}"
            )  # Log the error
